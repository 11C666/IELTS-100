...
const manual=require(path.join(root,'content','manual-days-004-020.js'));
...
const bundle='/* Static offline lesson data. Day 004–020 are hand-authored. */\n'+all.map(d=>`window.IELTS_DAYS[${d.id}]=${JSON.stringify(d)};`).join('\n')+'\n';
fs.writeFileSync(path.join(root,'js','generated-content.js'),bundle);
console.log('Published hand-authored Day 004–020 and rebuilt the static offline bundle.');
